...
class EstablecimientoControllerTest extends AbstractControllerTest {
	...
	void solicitamos_todos_los_establecimientos() throws Exception {
		...
		MvcResult respuesta = mockMvc.perform(get("/establecimientos")).andExpect(status().isOk()).andReturn();

		checkResponseBody(establecimientos, respuesta);
	...
	void solicitamos_establecimiento_EXISTENTE_a_partir_de_su_codigo() throws Exception {
		...
		MvcResult respuesta = mockMvc.perform(get("/establecimientos/100"))
											.andExpect(status().isOk())
											.andReturn();
		
		checkResponseBody(establecimiento1, respuesta);
	}
	...
	void solicitamos_establecimiento_NO_EXISTENTE_a_partir_de_su_codigo() throws Exception {
		...
		checkResponseBody(new RespuestaError("No existe el establecimiento 100"), respuesta);
	...
	void solicitamos_establecimiento_con_PARAMETRO_NO_VALIDO_a_partir_de_su_codigo() throws Exception {
		...
		MvcResult respuesta = mockMvc.perform(get("/establecimientos/hola")).andExpect(status().isBadRequest())
				.andReturn();

		RespuestaError respuestaError = new RespuestaError(
				"El parámetro hola es de tipo java.lang.String - Se requiere un parámetro de tipo Long");

		checkResponseBody(respuestaError, respuesta);
	}
	...
	void creamos_establecimiento_con_codigo_NO_null() throws Exception {
		...
		MvcResult respuesta = mockMvc.perform(post("/establecimientos").content(requestBody).contentType("application/json"))
